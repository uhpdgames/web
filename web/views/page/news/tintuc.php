					...
					<div id="collapseExample" class="list-news-latest layered collapse show">
						...
						<div class="w-100 item-article clearfix">
							...
							if (is_array($news) && count($news)) {
								foreach ($news as $k => $v) {
									$v['url'] = str_replace('tin-tuc','noi-bo', $v['url']);
									$v['url'] = 'noi-bo' . '/' .$v['url'];
									$v['url'] = str_replace('noi-bo/noi-bo','noi-bo', $v['url']);
									...
									<div class="w-100 d-flex flex-row flex-wrap h-rem align-items-center text-left">
										...
										<div class="post-content">
											<h3><a class="name_post catchuoi2"
												   href="<?php echo $v['url'] ?>"><?= ucfirst(mb_strtolower($v['name'])) ?></a>
											</h3>

											<span
												class="name_post catchuoi3"><?= strip_tags_content($v['description'] ?? '') ?></span>
				...
				<div class="list-group">
					...
					if (is_array($item) && count($item)) {
						foreach ($item as $value) {
							...
							$value['url'] = str_replace('tin-tuc','noi-bo', $value['url']);
							$value['url'] = 'noi-bo' . '/' .$value['url'];
							$value['url'] = str_replace('noi-bo/noi-bo','noi-bo', $value['url']);

							$value['url'] .= '?details=' . @$value['id'];

							?>
							<div class="item_news m-2 m-md-0 p-0">
								<div class="row m-0 p-0">
									<div class="col-md-6 m-0 p-0">
										<a href="<?= $value['url'] ?>" target="_self"
										>
